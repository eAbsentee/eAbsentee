...
with SSHClient() as ssh:
    ...
    count_stdin, count_stdout, count_stderr = ssh.exec_command(f"mysql -u {username} -h {username}.mysql.pythonanywhere-services.com -e 'SELECT email FROM admin_user UNION SELECT email FROM group_references' --skip-column-names '{username}$users'")
